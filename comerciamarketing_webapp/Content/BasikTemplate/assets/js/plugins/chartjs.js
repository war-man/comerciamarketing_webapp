(function (global, theme) {
  ...
  Chart.defaults.easypie = Chart.defaults.pie;
  var easypie = Chart.controllers.pie.extend({
      draw: function(ease) {
        var me = this;
        var chart = me.chart;
        ...
        var minSize = Math.min(chart.width, chart.height) - borderWidth;
        var centerX = chart.width / 2;
        var centerY = chart.height / 2;
        var percent = Math.min(Math.max(-1, dataset.data[0]/100 || 0), 1);
        var isNegative = percent <= 0 ? true : false;
        var angleInRadians = percent*360 * Math.PI / 180.0;
        var x = 0 + minSize/2 * Math.cos(angleInRadians);
        var y = 0 + minSize/2 * Math.sin(angleInRadians);
        var gradient = ctx.createLinearGradient(0,0,x,y);
        Chart.helpers.each(dataset.borderColor, function(color, index) {
          gradient.addColorStop(index/(dataset.borderColor.length - 1), color);
        });
        ctx.save();

        ctx.translate(centerX, centerY);
        ctx.rotate((-1 / 2 + (opts.rotate || 0) / 180) * Math.PI);

        this.drawCircle(ctx, minSize/2, dataset.backgroundColor, borderWidth, 1);
        this.drawCircle(ctx, minSize/2, gradient, borderWidth, percent*ease);
        ctx.restore();
      },
      drawCircle: function(ctx, radius, color, lineWidth, percent) {
  ...
  Chart.controllers.easypie = easypie;

  var gradientPlugin = {
    beforeDatasetUpdate: function(chart) {
      if (chart.options.gradient) {
        var ctx = chart.ctx;
        var width = chart.width;
        var height = chart.height;
        var datasets = chart.data.datasets;
        var type = chart.config.type;
        Chart.helpers.each(datasets, function(dataset) {
          var colors = dataset.gradient || dataset.borderColor;
          var w = width;
          var h = 0;
          if(type == 'bar' || type == "horizontalBar"){
            colors = dataset.gradient || dataset.backgroundColor;
          }
          if(chart.options.gradient == 'v'){
            w = 0;
            h = height;
          }
          dataset.gradient = colors;
          if(typeof colors !== 'string'){
            var gradient = ctx.createLinearGradient(0, 0, w, h);
            Chart.helpers.each(colors, function(color, index) {
  ...
  Chart.pluginService.register(gradientPlugin);
